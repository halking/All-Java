...
public class KeyWordData extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		List<ReportKeyMatchTopDto> dtos = (List<ReportKeyMatchTopDto>)model.get("dtos");
		Locale locale = RequestContextUtils.getLocale(request);
		String name = I18nUtil.getMessage("report.keyword", locale);
		HSSFSheet sheet = workbook.createSheet(name);
		sheet.setDefaultColumnWidth(3);
		HSSFRow title = sheet.createRow(0);
		String[] keys = {"no", "report.keyword", "report.trigger.amount"};
		...
		if(dtos!=null) {
			...
			for (int i = 0; i < dtos.size(); i++) {
				ReportKeyMatchTopDto dto = dtos.get(i);
				HSSFRow dataRow = sheet.createRow(rowId);
				dataRow.setHeight((short) 300);
				dataRow.createCell(0).setCellValue(rowId);
				dataRow.createCell(1).setCellValue(dto.getContent());
				dataRow.createCell(2).setCellValue(dto.getCount());
				rowId++;
			}
		}

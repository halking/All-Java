...
import com.d1m.wechat.wxsdk.core.req.model.message.AutoReplyRuleGet;
import com.d1m.wechat.wxsdk.core.sendmsg.model.WxArticleConfig;
import com.d1m.wechat.wxsdk.core.sendmsg.model.auto.AutoReplyInfoRule;
import com.d1m.wechat.wxsdk.core.sendmsg.model.auto.KeyWordAutoReplyInfo;
import com.d1m.wechat.wxsdk.core.sendmsg.model.auto.KeywordListInfo;
import com.d1m.wechat.wxsdk.core.sendmsg.model.auto.ReplyListInfo;
...
public class JwGetAutoReplyRuleAPI {
	private static Logger logger = LoggerFactory
			.getLogger(JwGetAutoReplyRuleAPI.class);
	 ...
	 * @return
	 */
	public static AutoReplyInfoRule getAutoReplyInfoRule(String accessToken)
		...
		JSONObject result = WeiXinReqService.getInstance()
				.doWeinxinReqJson(arr);
		Object error = result.get(WeiXinConstant.RETURN_ERROR_INFO_CODE);

		AutoReplyInfoRule autoReplyInfoRule = (AutoReplyInfoRule) JSONObject
				.toBean(result, new CustomJsonConfig(AutoReplyInfoRule.class,
						"keyword_autoreply_info"));
		JSONObject keywordAutoReplyInfoJsonObj = result
				.getJSONObject("keyword_autoreply_info");
		if (keywordAutoReplyInfoJsonObj != null
				&& !JSONUtils.isNull(keywordAutoReplyInfoJsonObj)) {
			/** 关键词自动回复的信息 */
			JSONArray keywordAutoReplyInfos = keywordAutoReplyInfoJsonObj
					.getJSONArray("list");
			if (keywordAutoReplyInfos != null) {
				...
				for (int i = 0; i < keywordAutoReplyInfos.size(); i++) {
					KeyWordAutoReplyInfo keyWordAutoReplyInfo = (KeyWordAutoReplyInfo) JSONObject
							.toBean(keywordAutoReplyInfos.getJSONObject(i),
									new CustomJsonConfig(
											KeyWordAutoReplyInfo.class,
											new String[] { "keyword_list_info",
													"reply_list_info" }));
					/** 处理关键词列表 */
					JSONArray keywordListInfos = keywordAutoReplyInfos
							.getJSONObject(i).getJSONArray("keyword_list_info");
					if (keywordListInfos != null) {
						...
						for (int j = 0; j < keywordListInfos.size(); j++) {
							KeywordListInfo keywordListInfo = (KeywordListInfo) JSONObject
									.toBean(keywordListInfos.getJSONObject(j),
											KeywordListInfo.class);
							listKeywordListInfo.add(keywordListInfo);
						}
						keyWordAutoReplyInfo
								.setKeyword_list_info(listKeywordListInfo);
					}

					/** 处理关键字回复信息 */
					JSONArray replyListInfos = keywordAutoReplyInfos
							.getJSONObject(i).getJSONArray("reply_list_info");
					if (replyListInfos != null) {
						...
						for (int j = 0; j < replyListInfos.size(); j++) {
							ReplyListInfo replyListInfo = (ReplyListInfo) JSONObject
									.toBean(keywordListInfos.getJSONObject(j),
											new CustomJsonConfig(
													ReplyListInfo.class,
													"news_info"));
							/** 处理关键字回复相关图文消息 */
							JSONObject newsInfoJsonObj = replyListInfos
									.getJSONObject(j)
						...
						keyWordAutoReplyInfo
								.setReply_list_info(listReplyListInfo);
					}

					listKeyWordAutoReplyInfo.add(keyWordAutoReplyInfo);
				}
				autoReplyInfoRule
						.setKeyword_autoreply_info(listKeyWordAutoReplyInfo);
			}
		}

package com.d1m.wechat.wxsdk.core.util;
...
public class WeiXinReqUtil {
	...
	private static Map<String, String> CONTENT_TYPE_FILE = new HashMap<String, String>();
	...
	public static void initReqConfig(String configName) throws JDOMException,
		...
		for (Element mapping : lstMapping) {
			...
			String mappingHandler = "com.d1m.wechat.wxsdk.core.handler.impl.WeixinReqDefaultHandler";
	...
	private static void loadPropertiesFile() {
		if (file_content_type == null) {
			...
			try {
				...
				for (Entry<Object, Object> entry : file_content_type.entrySet()) {
					CONTENT_TYPE_FILE.put(entry.getValue().toString(), entry
							.getKey().toString());
				}
			} catch (IOException e) {
	 ...
	 * @return
	 */
	public static String getFileSuffix0(String contentType) {
		loadPropertiesFile();
		String fileSuffix = "";
	...
	public static String getFileSuffix(String contentType) {
		...
		loadPropertiesFile();
		return CONTENT_TYPE_FILE.get(contentType);
	}

	public static void main(String[] args) throws JDOMException, IOException {
		WeixinReqParam req = new WeixinReqParam();
		req.setAccess_token("http://mp.weixin.qq.com/s?__biz=MjM5MzYzMTMyMg==&mid=406368323&idx=1&sn=10afceb5edbe5f187ef3188a2bb0abc7#rd");
		System.out.println(WeiXinReqUtil.getWeixinParamJson(req));
		/*
		 * Map<String,ActionConfig> mapActionConfig =
		 * MvcUtils.initStrutsConfig(); Set<String> actionSet =
		 * mapActionConfig.keySet(); for(Iterator it =
		 * actionSet.iterator();it.hasNext();){

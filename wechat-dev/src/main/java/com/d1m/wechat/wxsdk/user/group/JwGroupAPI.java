package com.d1m.wechat.wxsdk.user.group;
...
import com.d1m.wechat.wxsdk.core.req.model.user.BatchGroupMembersUpdate;
import com.d1m.wechat.wxsdk.core.req.model.user.Group;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupCreate;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupDelete;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupGet;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupGetId;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupMembersUpdate;
import com.d1m.wechat.wxsdk.core.req.model.user.GroupUpdate;
...
public class JwGroupAPI {
	 ...
	 */
	public static GroupCreate createGroup(String accesstoken, String groupName)
			throws WechatException {
		...
		groupCreate = (GroupCreate) JSONObject
				.toBean(result, GroupCreate.class);
		return groupCreate;
	}
	...
	public static List<Group> getAllGroup(String accesstoken)
		...
		for (int i = 0; i < infoArray.size(); i++) {
			Group group = (Group) JSONObject.toBean(infoArray.getJSONObject(i),
					Group.class);
	 ...
	 * @throws WechatException
	 */
	public static String updateGroup(String accesstoken, String groupId,
			String groupNewName) throws WechatException {
	 ...
	 * @throws WechatException
	 */
	public static String groupMemberMove(String accesstoken, String openid,
			String to_groupid) throws WechatException {
		JSONObject result = groupMemberMove0(accesstoken, openid, to_groupid);
		return result.getString("errmsg");
	}

    public static JSONObject groupMemberMove0(String accesstoken, String openid,
                                              String to_groupid) throws WechatException {
        GroupMembersUpdate u = new GroupMembersUpdate();
        u.setAccess_token(accesstoken);
        u.setOpenid(openid);
        u.setTo_groupid(to_groupid);
        return WeiXinReqService.getInstance().doWeinxinReqJson(u);
    }
	 ...
	 * @throws WechatException
	 */
	public static String batchGroupMemberMove(String accesstoken,
			List<String> openid_list, String to_groupid) throws WechatException {
	...
	public static JSONObject batchGroupMemberMove0(String accesstoken,
			List<String> openid_list, String to_groupid) throws WechatException {
		BatchGroupMembersUpdate u = new BatchGroupMembersUpdate();
		u.setAccess_token(accesstoken);
		u.setOpenid_list(openid_list);
	 ...
	 * @param accesstoken
	 *            访问凭证
	 * @param deletegGroupid
	 *            删除的分组ID
	 */
	public static String groupDelete(String accesstoken, String deletegGroupid)
			throws WechatException {
		// 步骤一、api调用参数准备.
		...
		JSONObject result = WeiXinReqService.getInstance().doWeinxinReqJson(
				group);
		// 步骤三、对返回结果进行处理.
		// author machaolin by 20150715 for
		// 很奇怪微信服务器没有按接口说明返回{errcode:xxx,errmsg:xxx},而是返回了{}。
	...
	private static void testBatchGroupMemberMove() {
		...
		try {
			...
			String groupid = JwGroupAPI.batchGroupMemberMove(TOKEN, openidList,
					"101");
			System.out.println("该粉丝所在分组为:" + groupid);
		} catch (WechatException e) {
			e.printStackTrace();

...
public class WeixinReqMediaDownHandler implements WeiXinReqHandler {
	...
	private static Logger logger = Logger
			.getLogger(WeixinReqMediaDownHandler.class);
	...
	public String doRequest(WeixinReqParam weixinReqParam)
		...
		if (weixinReqParam.getClass().isAnnotationPresent(ReqType.class)) {
			...
			if (objConfig != null) {
				...
				logger.info("reqUrl : " + reqUrl + ", filePath : " + filePath);
				strReturnInfo = HttpRequestProxy.downMadGet(reqUrl, parameters,
						"UTF-8", filePath, downloadMedia.getMedia_id());
			}
		} else {

package com.d1m.wechat.wxsdk.wxshop;
...
import com.d1m.wechat.wxsdk.core.common.JSONHelper;
import com.d1m.wechat.wxsdk.core.common.WxstoreUtils;
import com.d1m.wechat.wxsdk.wxshop.model.BaseInfo;
import com.d1m.wechat.wxsdk.wxshop.model.Business;
import com.d1m.wechat.wxsdk.wxshop.model.BusinessReq;
...
public class JwShopAPI {
	...
	public static String getShopCategorys(String newAccessToken) {
		if (newAccessToken != null) {
			String requestUrl = shop_category_url.replace("ACCESS_TOKEN",
					newAccessToken);
			JSONObject result = WxstoreUtils.httpRequest(newAccessToken, requestUrl, "GET",
					null);
	...
	public static ShopRtnInfo doAddshop(String newAccessToken, Business business) {
		if (newAccessToken != null) {
			String requestUrl = create_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			JSONObject obj = JSONObject.fromObject(business);
			JSONObject result = WxstoreUtils.httpRequest(newAccessToken, requestUrl, "POST",
					obj.toString());
			ShopRtnInfo shopRtnInfo = (ShopRtnInfo) JSONObject.toBean(result,
					ShopRtnInfo.class);
			return shopRtnInfo;
		}
		return null;
	 ...
	 * 修改门店服务信息
	 */
	public static ShopRtnInfo updateShop(String newAccessToken,
			Business business) {
		if (newAccessToken != null) {
			String requestUrl = updatebystatus_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			JSONObject obj = JSONObject.fromObject(business);
			JSONObject result = WxstoreUtils.httpRequest(newAccessToken, requestUrl, "POST",
					obj.toString());
	...
	public static ShopRtnInfo delShop(String newAccessToken, PoiId poiId) {
		if (newAccessToken != null) {
			String requestUrl = del_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
	...
	public static BaseInfo getshop(String newAccessToken, PoiId poiId) {
		if (newAccessToken != null) {
			String requestUrl = get_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			JSONObject obj = JSONObject.fromObject(poiId);
			JSONObject result = WxstoreUtils.httpRequest(newAccessToken, requestUrl, "POST",
					obj.toString());
	 ...
	 * 查询门店列表
	 */
	public static List<BaseInfo> getshops(String newAccessToken,
			BusinessReq businessReq) {
		if (newAccessToken != null) {
			String requestUrl = search_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
	 ...
	 * @return
	 */
	public static String uploadImg(String newAccessToken, String filePath) {
		if (newAccessToken != null) {
			String requestUrl = upload_img_shop_url.replace("ACCESS_TOKEN",
					newAccessToken);
			byte[] fileByte;
			try {
				fileByte = fileData(filePath);
				JSONObject result = WxstoreUtils.httpRequest2(newAccessToken, requestUrl,
						"POST", fileByte);
